...
BEGIN_DATADESC( CProp_Portal )
	...
	DEFINE_FIELD( m_frametime,			FIELD_FLOAT),
...
void CProp_Portal::detectMove( void )
{
	if ( relativeEntity )
	{
		if ( prevRelOrig != relativeEntity->GetAbsOrigin() )
		{
			portalVelocity = ( relativeEntity->GetAbsOrigin() - prevRelOrig ) / gpGlobals->frametime;
			prevRelOrig = relativeEntity->GetAbsOrigin();
		}
	}
	SetContextThink( &CProp_Portal::detectMove, gpGlobals->curtime + gpGlobals->frametime, "VelocityCheck" );
}
...
void CProp_Portal::findParent( void )
{
	...
	Ray_t ray;
	ray.Init( vOrigin, vOrigin - ( vForward*2 ) );
	...
	if ( tr.m_pEnt )
	{
		if ( FClassnameIs( tr.m_pEnt, "func_brush" ) || FClassnameIs( tr.m_pEnt, "func_tracktrain" ) || FClassnameIs( tr.m_pEnt, "func_physbox") )
		{
			relativeEntity = tr.m_pEnt;
			SetParent( relativeEntity );
		}
	}
	if ( tr.DidHitWorld() )
	{
...
void CProp_Portal::FindRelativeEntityThink( void )
{
	...
	if ( relativeEntity )
	{
		...
		RegisterThinkContext( "VelocityCheck" );
		SetContextThink( &CProp_Portal::detectMove, gpGlobals->curtime, "VelocityCheck" );
	}
}
...
void CProp_Portal::UpdatePortalThink( void )
{
	...
	WakeNearbyEntities();
	SetContextThink( &CProp_Portal::UpdatePortalThink, gpGlobals->curtime + ( m_frametime ), s_pFindRelativeEntityContext );
}

//-----------------------------------------------------------------------------
// Purpose: move the portal forward depending on the velocity vector to elliminate clipping
//-----------------------------------------------------------------------------
void CProp_Portal::MovePortalForward( void )
{
	Vector vForward;
	GetVectors( &vForward, NULL, NULL );
	AngleVectors( GetAbsAngles(), &vForward );
	if ( vForward.Dot( portalVelocity ) > 0 )
		SetLocalOrigin( vAttatch + ( vForward * ( ( vForward.Dot( portalVelocity.Normalized() ) * 3 ) ) ) );
}
...
Vector CProp_Portal::getPortalVelocity( void )
{
	if ( !relativeEntity )
		return vec3_origin;
	return portalVelocity;
}

bool CProp_Portal::ShouldTeleportTouchingEntity( CBaseEntity *pOther )
{
	//don't teleport the entity that's holding the portal
	if ( relativeEntity )
	{
		if ( pOther == relativeEntity )
			return false;
	}	
...
{
	//if this portal is trying to teleport the other portal, something's gone terribly wrong and the portal needs to go away
	if ( pOther == m_hLinkedPortal )
	{
		Fizzle();
	}
	if ( m_hLinkedPortal && m_hLinkedPortal->relativeEntity && ( pOther == m_hLinkedPortal->relativeEntity ) )
	{
		Fizzle();
	}
...
void CProp_Portal::EndTouch( CBaseEntity *pOther )
{
	...
	if ( pOther->VPhysicsGetObject() && relativeEntity && relativeEntity->VPhysicsGetObject() )
	{
		PhysEnableObjectCollisions( pOther->VPhysicsGetObject(), relativeEntity->VPhysicsGetObject() );
	}
	
	pOther->SetCollisionGroup( storeCollisionGroup );
	*/
...
void CProp_Portal::NewTranslation(const Vector &vOrigin)
{
	// Tell our physics environment to stop simulating it's entities.
	// Fast moving objects can pass through the hole this frame while it's in the old location.
	m_PortalSimulator.ReleaseAllEntityOwnership();
	...
	Teleport(&vOrigin, NULL, 0);

	if (m_hMicrophone)
	{
		CEnvMicrophone *pMicrophone = static_cast<CEnvMicrophone*>(m_hMicrophone.Get());
		pMicrophone->Teleport(&vOrigin, NULL, 0);
		inputdata_t in;
		pMicrophone->InputEnable(in);
	}
	...
	{
		CSpeaker *pSpeaker = static_cast<CSpeaker*>(m_hSpeaker.Get());
		pSpeaker->Teleport(&vOrigin, NULL, 0);
		inputdata_t in;
		pSpeaker->InputTurnOn(in);
	}
