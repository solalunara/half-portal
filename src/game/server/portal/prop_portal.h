...
public:
	...
	CBaseEntity* relativeEntity;
	Vector prevRelOrig;
	Vector portalVelocity;
	int storeCollisionGroup;
	Vector vAttatch;
	float m_frametime;
	...
	CPhysCollide			*createCollisionShape( void );
	...
	void					UpdatePortalThink( void );

	void					MovePortalForward( void );
	...
	void					NewTranslation( const Vector &vOrigin );
