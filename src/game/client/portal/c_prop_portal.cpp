...
IMPLEMENT_CLIENTCLASS_DT( C_Prop_Portal, DT_Prop_Portal, CProp_Portal )
	...
	RecvPropFloat( RECVINFO(m_frametime) ),
END_RECV_TABLE()
...
void C_Prop_Portal::ClientThink( void )
{
	...
	if ( m_frametime != gpGlobals->frametime )
	{
		m_frametime = gpGlobals->frametime;
		bDidAnything = true;
	}
...
void C_Prop_Portal::OnDataChanged( DataUpdateType_t updateType )
{
	...
	bool bPortalMoved = ( ( ( PreDataChanged.m_vOrigin - m_ptOrigin).Length() > 0 ) ||
						( ( PreDataChanged.m_qAngles - GetAbsAngles()).Length() > 0 ) || 
						( PreDataChanged.m_bActivated == false ) ||
						( PreDataChanged.m_bIsPortal2 != m_bIsPortal2 ) );
	bool bNotPortalMovedFar = false;
	if ( m_hLinkedPortal )
		bNotPortalMovedFar = ( ( ( PreDataChanged.m_vOrigin - m_ptOrigin ).Length() < 10 ) &&
						( ( PreDataChanged.m_qAngles - GetAbsAngles() ).Length() < 10 ) /*&&
						( PreDataChanged.m_bIsPortal2 == m_bIsPortal2 ) )/* &&
						( ( ( m_hLinkedPortal->PreDataChanged.m_vOrigin - m_hLinkedPortal->m_ptOrigin ).Length() < 100 ) &&
						( ( m_hLinkedPortal->PreDataChanged.m_qAngles - m_hLinkedPortal->GetAbsAngles() ).Length() < 100 ) &&
						( m_hLinkedPortal->PreDataChanged.m_bIsPortal2 == m_hLinkedPortal->m_bIsPortal2 ) )*/);
	...
	if( m_bActivated )
	{
		...
		{			
			...
			//TODO: if portal existed in close location last frame, m_fOpenAmount = 1.0f, else m_fOpenAmount = 0.0f;
			if ( !bNotPortalMovedFar )
				m_fOpenAmount = 0.0f;
			else
				m_fOpenAmount = 1.0f;
